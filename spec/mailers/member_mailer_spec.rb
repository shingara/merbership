...
module MailControllerTestHelper
  ...
  def deliver(action, mail_params = {}, send_params = {})
    MemberMailer.dispatch_and_deliver(action, { :from => "no-reply@webapp.com", :to => "recepient@person.com" }.merge(mail_params), send_params)
    @delivery = last_delivered_mail
  end
end

describe MemberMailer, "#notify_on_event email template" do
  ...
  it "includes welcome phrase in email text" do
    ...
    # MemberMailer.dispatch_and_deliver(:notify_on_event, {}, { :name => "merb-mailer user" })
